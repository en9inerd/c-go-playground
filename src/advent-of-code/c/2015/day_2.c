...
int main(int argc, char *argv[]) {
  ...
  unsigned int totalSquare = 0;
  unsigned int totalLengthOfRibbon = 0;

  // prism dimensions
  unsigned int length, width, height, sq1, sq2, sq3, minSq, minPerimeter;
  ...
  while (fgets(line, MAX_LINE_LENGTH, file) != NULL) {
    if (sscanf(line, "%dx%dx%d", &length, &width, &height) == 3) {
      ...
      sq1 = length * width;
      minSq = sq1;
      minPerimeter = length + width;

      sq2 = length * height;
      if (sq2 < minSq)
        minSq = sq2;
      if ((length + height) < minPerimeter)
        minPerimeter = length + height;

      sq3 = width * height;
      if (sq3 < minSq)
        minSq = sq3;
      if ((width + height) < minPerimeter)
        minPerimeter = width + height;

      // calculate length of ribbon
      totalLengthOfRibbon += 2 * minPerimeter + length * width * height;

      totalSquare += 2 * (sq1 + sq2 + sq3) + minSq;
    } else {
